...
export default function ComposeModal({ senderId: initialSenderId, onClose, onCreated }: Props) {
  ...
  async function createCampaign(e: React.FormEvent) {
    ...
    try {
      ...
      if (recipientList.length === 0) throw new Error("Add at least one valid recipient email.");
      ...
      <div style={styles.modal}>
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>Schedule and send an email campaign.</p>
          </div>
        ...
        <form onSubmit={createCampaign}>
          ...
          <input value={senderId} onChange={(e) => setSenderId(e.target.value)} placeholder="Enter sender ID" style={styles.input} />

          <label style={styles.label}>Subject</label>
          <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Email subject" required style={styles.input} />

          <label style={styles.label}>Email Body</label>
          <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your email..." required rows={7} style={styles.textarea} />

          <label style={styles.label}>Upload leads (CSV or text file)</label>
          <input type="file" accept=".csv,.txt" onChange={handleFileUpload} style={styles.input} />
          {fileName && (
            <p style={styles.hint}>
              Loaded <strong>{fileName}</strong> — {detectedCount} valid email address{detectedCount === 1 ? "" : "es"} detected.
            </p>
          )}

          <label style={styles.label}>Or paste recipients</label>
          <textarea
            value={recipients}
          ...
          />
          <p style={styles.hint}>{detectedCount} valid email address{detectedCount === 1 ? "" : "es"} detected.</p>
          ...
          <div style={styles.grid}>
            ...
            <div>
              ...
              <input type="number" min="1" value={delayMs} onChange={(e) => setDelayMs(e.target.value)} required style={styles.input} />
            </div>
            <div>
              <label style={styles.label}>Hourly Limit</label>
              <input type="number" min="1" value={hourlyLimit} onChange={(e) => setHourlyLimit(e.target.value)} required style={styles.input} />
            </div>
          </div>

          <div style={styles.footer}>
            <button type="button" onClick={onClose} style={styles.cancel} disabled={loading}>Cancel</button>
            <button type="submit" style={styles.submit} disabled={loading}>{loading ? "Creating..." : "Create Campaign"}</button>
          </div>
        </form>
      </div>
    </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  cancel: { border: "1px solid #d4d4d4", background: "#fff", padding: "10px 17px", borderRadius: "8px", cursor: "pointer" },
  submit: { border: "none", background: "#111", color: "#fff", padding: "10px 18px", borderRadius: "8px", cursor: "pointer" },
  error: { background: "#fee2e2", color: "#991b1b", padding: "11px", borderRadius: "8px", fontSize: "13px", marginBottom: "15px" },
};
